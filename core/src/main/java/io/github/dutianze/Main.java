...
public class Main extends ApplicationAdapter {
    ...
    public void create() {
        ...
        taskbar.background("taskbar");
        root.add(taskbar).growX();
        taskbar.pad(2.0f);

        TextButton taskbarButton = new TextButton("jar launcher", skin, "default");
        taskbar.add(taskbarButton).expandX().left().padLeft(5.0f);
        ...
        TextButton shutdownButton = new TextButton("Turn Off Computer", skin, "shutdown");
        ...
        table.add(shutdownButton).expandX().right().padRight(5.0f);
            ...
            public void onFileChosen(FileHandle file) {
                try {
                    ...
                    Gdx.app.postRunnable(() -> {
                        ...
                        appendToLog("JAR file uploaded: " + file.name() + " at " + getCurrentTime());
                    });
                } catch (IOException e) {
                    Gdx.app.postRunnable(() -> {
        ...
        try {
            ...
            new Thread(() -> {
                try (RandomAccessFile raf = new RandomAccessFile(LOG_PATH, "r")) {
                        ...
                        if (fileLength > filePointer) {
                            ...
                            while ((line = raf.readLine()) != null) {
                                ...
                                if (line.toLowerCase().contains("completed initialization")) {
                                    if (statusFinalized.compareAndSet(false, true)) {
                                        Gdx.app.postRunnable(
                                            () -> statusLabel.setText("Status: Server started on port " + port));
                                    }
                                }
                            }
                    ...
                    Gdx.app.postRunnable(() -> statusLabel.setText("Status: Server error - " + e.getMessage()));
    ...
    private void appendToLog(String message) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(LOG_PATH, true))) {
            ...
            Gdx.app.postRunnable(() -> statusLabel.setText("Status: Failed to write to log - " + e.getMessage()));
        }
    }
